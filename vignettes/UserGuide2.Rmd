...
The `bycatchFit` function provides many options for the observation error model groups (modelTry) to allow users flexibility in setting up models. However, many of these models (e.g. normal) are unlikely to be effective. In practice, negative binomial (1 or 2), or delta-lognormal and Tweedie work for most applications. This section provides more detials on how the model fit and selection works, with guidance for selecting models. 

#### GLM model types

In general, the generalized linaer models (GLM) vary on whether they assume the y data will be 0 and 1 (binomial), counts (negative binomial), or real numbers (all others), how they model the mean and how they model the variance of the predictions. 
...
The variance is an estimated constant for the normal and lognormal. The negative binomial using the glm.nb function from the MASS library (@MASS) or nbinom2 from the glmmTMB library(@glmmTMB). are the ordinary negative binomial, in which the variance: $$\sigma^2=\mu+\mu^2/\theta$$ where $\theta$ is an estimated parameter. For nbinom1, the variance is defined as: $$\sigma^2= \mu(1+\alpha)$$ where $\alpha$ is an estimated parameter. This version of the negative binomial model, which is equivalent to a quasi-Poisson model, gives somewhat different results from the other negative binomial models. The values of theta and alpha are given in the "scale" column in the parameter summary table. 
