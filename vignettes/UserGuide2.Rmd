...
The BycatchEstimator tool estimates total bycatch, calculated by expanding a sample, such as an observer database, to total effort from logbooks or landings records. The tool can be used to run eitehr a generic model-based bycatch estimation procedure, or common design-based bycatch estimation methods, after you first set up the data inputs and other specifications. 
...
To include the observed catches as known in the totals for model-based methods, it is necessary to include a column for unsampled effort in the logbook data (which can be zero in completely sampled trips) and a column in both the logbook and observer data that can be used to match  observed trips to logbook records. If you are not using this feature, set includeObsCatch to FALSE.  

This function outputs the user's choice of an html or a pdf file, including summary figures and tables. There will be warning messages if there are any NA values in the data, or if levels of factor levels in logbook data are not present in the observer data. The following plots are produced.  
