...
- modpredVals, a list for all species, containing a list of tibbles of the bycatch predictions accross models, appropriate for plotting.

- allmods, a long format tibble with all the model annual predictions together, appropriate for plotting
...
- yearSumGraph, annual summaries in the same formula as the model outputs in allmods, convenient for combining

For example, the following code would read the results of design and modelFit objects, and make a plot combining the annual estimates. 

```{r,eval=evalOption}
library(tidyverse) #for data manipulation and ggplot
#Load in both design and model based estimates
loadOutputs(baseDir = getwd(),
                      runName= "Simulated",
                      runDate =  Sys.Date(),
                      loadDesign = TRUE,
                      designScenario = "withPooling",
...
spnum<-1
#combine model and design estimates, first making Year numeric
allResults<-bind_rows(yearSumGraph[[spnum]],
                      mutate(allmods[[spnum]],Year=as.numeric(as.character(Year))))

#Plot all together
ggplot(allResults,aes(x=Year,y=Total,ymin=TotalLCI,ymax=TotalUCI,
                      fill=Source,color=Source))+
  geom_line()+
  geom_ribbon(alpha=0.4)+
  labs(x="Year",
       y=paste0(common[spnum]," ",catchType[spnum]," (",catchUnit[spnum],")"))+
  theme_bw()
