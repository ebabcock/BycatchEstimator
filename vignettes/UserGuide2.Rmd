...
date: "`r Sys.Date()`"
vignette: "%\\VignetteIndexEntry{UserGuide} %\\VignetteEngine{knitr::rmarkdown} %\\VignetteEncoding{UTF-8}\n"
...
The design-based estimators are calculated when bycatchSetup is run, so you may stop here if you only need the design based estimators. They are given in the columns called ratioMean, ratiaSE, deltaMean and deltaSE in the file labelled DesignSummary and the file labelled DesignStrata. See details on model outputs below. See the function getPooling for details on how the pooling is set up for design-based estimation, and getDesignEstimates for the total bycatch calculations.

## Data checking

This provides guidance and summarizes arguments of the function `dataCheck`

The dataCheck function takes as input, the output from bycatchSetup.  It makes plots and summary tables including:
...
- Cat: estimated catch using ratio estimate (observed catch divided by observed effort raised by logbook effort)
