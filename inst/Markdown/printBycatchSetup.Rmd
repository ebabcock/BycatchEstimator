...
if(includeObsCatch & EstimateBycatch) {
     ...
     if(length(missing_trips)>0){
       cat("\n")
       cat(paste("The following sample units from the observer data are missing in the logbook data: "),paste(missing_trips,collapse = ", "),
...
temp<-obsdatTemp %>%
  mutate_at(all_of(c(numericVariables,"Effort")),cut,breaks=30,labels=FALSE)
 cat("\n\n")
