...
if(nrowstemp<=10)
 print(kbl(temp,format="simple")) else  {
   print(kbl(slice_head(temp,n=10),format="simple")) 
  cat("\n")
  cat(paste0("- plus ",nrowstemp-10," more rows."))   
  cat("\n")
 }
  cat("\n")
}
...
tablenum<-tablenum+1
obstemp<-select(dat[[run]],Effort) %>% mutate(SampleUnits=1)
temp_bind<-bind_rows(list(observer=obstemp,
                  logbook=filter(select(logdat,Effort,SampleUnits),!is.na(Effort))),
                  .id = "Source")

temp1 <- temp_bind  %>% group_by(Source) %>%
  summarize(`Sample units` = sum(SampleUnits),
...
cat("\n\n")
plot_effort_sampled(obsdat=obsdat,logdat=logdat,bin_size=spatialBin) 
cat("\n Figure ",figurenum,". Count of total ",sampleUnit," by ", spatialBin," x ",spatialBin," grid, and fraction observed (circles). \n",sep="")
figurenum<-figurenum+1
cat('<div style="height:30px;"></div>\n') 
}
```
...
if(!is.null(logdat)){
nmax<-5000  #If there are more data points than this, subsample to avoid hanging R
...
if(nrow(allData)>nmax) allData<-slice_sample(allData,n=nmax)
...
if(!is.null(latitudeVar) & !is.null(longitudeVar)) {
cat("\n\n")
plot_catch_sampled(obsdat=obsdat,bin_size=spatialBin,catch_col = obsCatch[[run]]) 
cat("\n Figure ",figurenum,". Count of observed ",sampleUnit," by ", spatialBin," x ",spatialBin," grid, and catch of ",common[[run]], " (circles). \n",sep="")
figurenum<-figurenum+1
cat('<div style="height:30px;"></div>\n') 
}
...
cat("\n\n")
temp<-obsdatTemp
if(nrow(temp)>nmax) temp<-slice_sample(temp,n=nmax)
temp<-temp%>% pivot_longer(cols=all_of(numericVariables),names_to="Variable",values_to = "Value") 
print(ggplot(temp,aes(x=Value,y=Catch))+
...
# log CPUE histogram
cat("\n\n")
cat("\n Figure ",figurenum,". Log of Observed CPUE plus a constant histogram for ",common[[run]], ".\n",sep="")
figurenum<-figurenum+1
