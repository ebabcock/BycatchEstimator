...
# out.dir <- "C:/Users/aadao/OneDrive/Documents/NA work 2025/ICCAT work/Tool improvements project/Output LLSIMBUMtripExample_tripmatching/"
# setupObj<-readRDS(file=paste0(out.dir,"2025-05-20_BycatchSetupSpecification.rds"))
# run=1
...
```{r NA-checks, results=FALSE,  message=TRUE, echo=FALSE, warning=TRUE}
...
```{r dataWarnings,results=TRUE,  message=FALSE, echo=FALSE, warning=FALSE,results="asis"}
if(length(factorVariables)>0 & !is.null(factorVariables)) {
  ...
  for(i in 1:length(factorVariables)) {
    ...
    if(!all(obsx %in% logx)) cat(paste0("Variable ",factorVariables[i]), "levels: ",
                                 obsx[!obsx %in% logx], "found in observer data but not logbook data; this may be a data error. ")
    if(!all(logx %in% obsx)) cat(paste0("Variable ",factorVariables[i]), "levels:",
                                 logx[!logx %in% obsx], "found in logbook data but not observer data; coefficients cannot be estimated for these levels. ")
  }
  cat("\n")
}
if(length(numericVariables)>0 & !is.null(numericVariables) &!is.na(numericVariables)) {
  for(i in 1:length(numericVariables)) {
    ...
    if(numobs>0) cat(paste0("Variable ",numericVariables[i]), "has",
    ...
    if(numlog>0) cat(paste0("Variable ",numericVariables[i]), "has",
...
print(ggplot(temp,aes(x=Level,y=Catch))+
  ...
  facet_wrap(Variable~.,scales="free")+
  stat_summary())
cat("\n Figure ",figurenum,". Observed Catch of ",common[[run]], " by factor levels.\n",sep="")
figurenum<-figurenum+1
...
print(ggplot(temp,aes(x=Value,y=Catch))+
  ...
  stat_smooth()+
  facet_wrap(Variable~.,scales="free"))
cat("\n Figure ",figurenum,". Observed Catch of ",common[[run]], " across numerical variables. \n",sep="")
figurenum<-figurenum+1
}
cat("\n")
