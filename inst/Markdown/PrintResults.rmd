...
if(modelFail[run,mod]=="-") {
  ...
  if(EstimateBycatch) {
   if(plotValidation & modelTry[mod]!="Binomial")  plotSumsValidate(dplyr::filter(allmods[[run]],Source==modelTry[mod]),trueVals,NULL,trueCols[run], allVarNames=allVarNames, startYear=startYear, common=common, run=run, catchType=catchType, catchUnit=catchUnit) else
