...
modelTry<-randomEffects<-randomEffects2<-complexModel<-simpleModel<-indexModel<-
  selectCriteria<-DoCrossValidation<-CIval<-VarCalc<-useParallel<-nSims<-predictionGroups<-
  plotValidation<-trueVals<-trueCols<-logUnsampledEffort<-includeObsCatch<-
  matchColumn<-EstimateIndex<-NULL

modelScenario<-modelTable<-modelSelectTable<-modelSummaryTable<-modFits<-modPredVals<-modIndexVals<-indexDat<-indexVarNames<-residualTab<-bestmod<-predbestmod<-indexbestmod<-allmodsStrata<-allmods<-allindex<-modelFail<-rmsetab<-metab<-dat<-requiredVarNames<-allVarNames<-startYear<-NumCores<-obsdatFit<-NULL
...
if(length(predictionGroups>1)) {
  cat("- Predictions were made for each combination of",paste(predictionGroups,collapse=", "),".")
cat("\n")
cat("\n")
}
...
if(DoCrossValidation) 
  if(length(modelTry)==1) cat("- Cross-validation not run for only one model type.") else
    if(!is.null(rmsetab[[run]])) {
      if(!all(is.na(rmsetab[[run]])))  cat("- Cross-validation results generated") else {
   cat("- Cross-validation failed") 
   DoCrossValidation<-FALSE  
 } 
}
if(DoCrossValidation & is.null(rmsetab[[run]])) {
  cat("- Cross-validation failed")
  DoCrossValidation<-FALSE  
}
cat("\n")
...
cat("Table ",tablenum,". Formula of ",selectCriteria," best model, along whether models were fit successfully. A dash (-) means the model converged. Failure to converge may be from data (not all years had a positive observation for delta models), fit (models did not converge) or CV (bycatch estimates had very large CVs).",sep="" )
if(DoCrossValidation)   cat(" The cross-validation RMSE and mean ME across folds are shown (near zero is better).",sep="")
tablenum<-tablenum+1
df1<-modelTable[[run]] %>%
...
if(EstimateBycatch & !all(allmods[[run]]$Source %in% c("Binomial","TMBbinomial")) ) {
 if(plotValidation)  {
...
if(length(predictionGroups)>1& !all(allmods[[run]]$Source %in% c("Binomial","TMBbinomial")) ) {
  cat('\n')
  cat('<div style="height:30px;"></div>\n') 
  plotSums(
    yearpred = filter(allmodsStrata[[run]],Valid==1),
    modType = "All",
    facetGroup = predictionGroups[!predictionGroups %in% c("Year")],
    fileName = NULL, allVarNames = allVarNames, startYear = startYear, common = common, run = run, catchType = catchType, catchUnit = catchUnit, VarCal = VarCalc)
  cat("Figure ",fignum,". Total bycatch estimates for all valid models, for ",common[run],varText,obsText,", by strata. \n",sep="")
  fignum<-fignum+1
 cat('<div style="height:30px;"></div>\n') 
}
...
cat('<div style="height:30px;"></div>\n') 
  ...
  if(EstimateBycatch) {
   if(plotValidation & ! modelTry[mod] %in% c("Binomial","TMBbinomial"))  
     plotSumsValidate(dplyr::filter(allmods[[run]],Source==modelTry[mod]),trueVals,NULL,trueCols[run],
                      allVarNames=allVarNames, startYear=startYear, common=common, run=run, 
                      catchType=catchType, catchUnit=catchUnit,VarCalc = VarCalc) else
   plotSums(modPredVals[[run]][[modelTry[mod]]],modelTry[mod],fileName=NULL, 
            allVarNames = allVarNames, startYear = startYear, common = common, 
            run = run, catchType = catchType, catchUnit = catchUnit)
  if(modelTry[mod] %in% c("Binomial","TMBbinomial"))  cat("\n Figure ",fignum,". Estimated total number of positive ",sampleUnit," from Binomial",varText,lineText,". \n",sep="") else
   ...
   cat('<div style="height:30px;"></div>\n') 
  if(length(predictionGroups)>1) {
   plotSums(modPredValsStrata[[run]][[modelTry[mod]]],modelTry[mod],fileName=NULL,
                facetGroup = predictionGroups[!predictionGroups %in% c("Year")],
            allVarNames = allVarNames, startYear = startYear, common = common, 
            run = run, catchType = catchType, catchUnit = catchUnit)
  if(modelTry[mod] %in% c("Binomial","TMBbinomial"))  cat("\n Figure ",fignum,". Estimated total number of positive ",sampleUnit," by strata, from Binomial",varText,lineText,". \n",sep="") else
   cat("\n Figure ",fignum,". Estimated total bycatch by strata, from ",modelTry[mod],varText,obsText,lineText, ". \n",sep="")
   fignum<-fignum+1
   cat('\n')
   cat('<div style="height:30px;"></div>\n')    
  } 
