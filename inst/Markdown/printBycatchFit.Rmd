...
# modelObj<-readRDS(file=paste0(out.dir,"2025-06-11_BycatchFitSpecification.rds"))
# run=1
...
cat('<div style="height:30px;"></div>\n') 
  ...
  if(!modelTry[mod] %in% c("Binomial")){
    if(modelTry[mod] %in% c("Tweedie")){
          tempdf<-data.frame(Expected=cplm::predict(modFits[[run]][[modelTry[mod]]],type="response"),
  ...
  print(ggplot(tempdf, aes(x = Expected, y = Observed))+
